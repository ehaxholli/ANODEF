...
class HyperConv2d(nn.Module):
    def __init__(self, dim_in, dim_out, ksize=3, stride=1, padding=0, dilation=1, groups=1, bias=True, transpose=False):
        ...
        self.mixdim=10
        self.mixer = nn.Linear(self.mixdim,self.mixdim)
        self.mixer2 = nn.Linear(self.mixdim,self.mixdim)
        self._hypernet = nn.Linear(self.mixdim,self.params_dim)
        self.elu=nn.ELU()
    ...
    def forward(self, da, x):
        da=da.view(1,-1)[:,:self.mixdim]
        params=self._hypernet(da).view(-1)
