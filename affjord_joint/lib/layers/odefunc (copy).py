...
class Aug_Cnn(nn.Module):
    """
    Determines the evolution of the augmented channel
    """

    def __init__(self, input_shape):
        self.input_shape=input_shape
        super(Aug_Cnn, self).__init__()
        self.itr=0
        self.c=self.input_shape[0]//2
        ...
        self.latdim=20
        self.fc1=nn.Linear(self.latdim,self.latdim)
        self.fc2=nn.Linear(self.latdim,self.latdim)
        self.actvf=nn.Tanh()
                
    def forward(self, xb):

        xb=xb.view(1,-1)[:,:self.latdim]
        xb=self.fc1(xb)
        xb=self.actvf(xb)
        xb=self.fc2(xb).squeeze()
        xb=torch.cat((xb,torch.zeros(self.c*self.h*self.w-self.latdim).to(xb))).reshape(self.c,self.h,self.w)
...
class ODEnet(nn.Module):
    ...
    def __init__(
        ...
        self.aug_cnn= Aug_Cnn(input_shape)
        if conv:
    ...
    def forward(self, t, y):
        ...
        if y.shape[1]==2:
            da = y[0, 0, :, :].reshape(1, 1, y.shape[2], y.shape[3])
            dx = y[:, 1, :, :].reshape(y.shape[0], 1, y.shape[2], y.shape[3])
            dx = torch.cat((torch.zeros(y.shape[0],1,y.shape[2],y.shape[3]).to(dx),dx),1)
        else:
            da = y[0, :y.shape[1]//2, :, :].reshape(1, y.shape[1]//2, y.shape[2], y.shape[3])
            dx = y[:, y.shape[1]//2:, :, :]
            dx = torch.cat((torch.zeros(y.shape[0],y.shape[1]//2,y.shape[2],y.shape[3]).to(dx),dx),1)
        ...
        if y.shape[1]==2:
            dx = dx[:, 1, :, :].reshape(y.shape[0], 1, y.shape[2], y.shape[3])
        else:
            dx = dx[:, :y.shape[1]//2, :, :].reshape(y.shape[0], y.shape[1]//2, y.shape[2], y.shape[3])
            
        da = self.aug_cnn(da)
        da=da.repeat(y.shape[0],1,1,1)
...
class ODEfunc(nn.Module):
    ...
    def __init__(self, diffeq, divergence_fn="approximate", residual=False, rademacher=False):
        ...
        self.randomname=np.random.rand(1)
    ...
    def forward(self, t, states):
        ...
        if y.shape[1]==2:
            aug, y_main= y[:,0,:,:], y[:,1,:,:]
            aug, y_main=aug.reshape(y.shape[0], 1, y.shape[2], y.shape[3]),\
                        y_main.reshape(y.shape[0], 1, y.shape[2], y.shape[3])
        else:
            aug, y_main= y[:,:y.shape[1]//2,:,:], y[:,y.shape[1]//2:,:,:]
            aug, y_main=aug.reshape(y.shape[0], y.shape[1]//2, y.shape[2], y.shape[3]),\
                        y_main.reshape(y.shape[0], y.shape[1]//2, y.shape[2], y.shape[3])
        # Sample and fix the noise.
        if self._e is None:
            if self.rademacher:
                self._e = sample_rademacher_like(y_main)
            else:
                self._e = sample_gaussian_like(y_main)

        with torch.set_grad_enabled(True):

            y_main.requires_grad_(True)
            t.requires_grad_(True)
            y = torch.cat((aug, y_main), 1)
            for s_ in states[2:]:
                s_.requires_grad_(True)
            da, dy = self.diffeq(t, y, *states[2:])

            # Hack for 2D data to use brute force divergence computation.
            if not self.training and dy.view(dy.shape[0], -1).shape[1] == 2:
                divergence = divergence_bf(dy, y_main).view(batchsize, 1)
            else:
                divergence = self.divergence_fn(dy, y_main, e=self._e).view(batchsize, 1)
        if self.residual:
            dy = dy - y
            divergence -= torch.ones_like(divergence) * torch.tensor(np.prod(y.shape[1:]), dtype=torch.float32
                                                                     ).to(divergence)

        dy = torch.cat((da, dy), 1)
