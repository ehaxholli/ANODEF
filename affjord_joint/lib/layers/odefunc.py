...
class ODEnet(nn.Module):
    ...
    def forward(self, t, y):
        ...
        if y.shape[1]==2:
            da = y[0, 0, :, :].reshape(1, 1, whdim, whdim)
            dx = y[:, 1, :, :].reshape(batch_size, 1, whdim, whdim)
            dx = torch.cat((torch.zeros(batch_size, 1, whdim, whdim).to(dx),dx),1)
        else:
            da = y[0, :cdim//2, :, :].reshape(1, cdim//2, whdim, whdim)
            dx = y[:, cdim//2:, :, :]
            dx = torch.cat((torch.zeros(batch_size, cdim//2, whdim, whdim).to(dx),dx),1)
        ...
        if y.shape[1]==2:
            dx = dx[:, 1, :, :].reshape(batch_size, 1, whdim, whdim)
        else:
            dx = dx[:, :cdim//2, :, :].reshape(batch_size, cdim//2, whdim, whdim)
            
        da = self.aug_cnn(da)
        da=da.repeat(batch_size,1,1,1)
...
class ODEfunc(nn.Module):
    ...
    def forward(self, t, states):
        ...
        if y.shape[1]==2:
            aug, y_main= y[:,0,:,:], y[:,1,:,:]
            aug, y_main=aug.reshape(batchsize, 1, whdim, whdim),\
                        y_main.reshape(batchsize, 1, whdim, whdim)
        else:
            aug, y_main= y[:,:cdim//2,:,:], y[:,cdim//2:,:,:]
            aug, y_main=aug.reshape(batchsize, cdim//2, whdim, whdim),\
                        y_main.reshape(batchsize, cdim//2, whdim, whdim)
