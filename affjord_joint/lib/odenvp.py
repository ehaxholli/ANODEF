...
class ODENVP(nn.Module):
    ...
    def __init__(
        ...
        self.aug_hist = [i for i in range(len(self.transforms))]
    ...
    def _logdensity(self, x, logpx=None):
        ...
        for idx in range(len(self.transforms)):
            ...
            self.aug_hist[idx], x= x[:,:x.shape[1]//2], x[:,x.shape[1]//2:]
    ...
    def _generate(self, z, logpz=None):
        ...
        for dims in self.dims:
            ...
            zs.append(z[:, i:i + s])
            i += s
        zs = [_z.view(_z.size()[0], zsize[0]//2, zsize[1], zsize[2]) for _z, zsize in zip(zs, self.dims)]
        _logpz = torch.zeros(zs[0].shape[0], 1).to(zs[0]) if logpz is None else logpz
        z_prev, _logpz = self.transforms[-1](torch.cat((self.aug_hist[-1][0].repeat(zs[-1].shape[0],1,1,1),zs[-1]),1), _logpz, reverse=True)
        for idx in range(len(self.transforms) - 2, -1, -1):
            z_prev = z_prev[:,z_prev.shape[1]//2:]
            z_prev = torch.cat((z_prev, zs[idx]), dim=1)
            z_prev = torch.cat((self.aug_hist[idx][0].repeat(z_prev.shape[0],1,1,1),z_prev),1)
...
class StackedCNFLayers(layers.SequentialFlow):
    def __init__(
        ...
        def _make_odefunc(size):
            net = ODEnet(idims, size, strides, True, layer_type=self.layertype, nonlinearity=nonlinearity)
